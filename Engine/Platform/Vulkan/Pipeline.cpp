...
namespace VT::Vulkan
{
    ...
    void GraphicsPipeline::Create(GraphicsPipelineCreateInfo CreateInfo)
    {
        ...
        vk::PipelineDepthStencilStateCreateInfo DepthStencilStateInfo{
            ...
            .front = ToVk(DepthStencilInfo.Front),
            .back = ToVk(DepthStencilInfo.Back),
        ...
        for (const auto& ShaderInfo : CreateInfo.Shader)
        {
            const auto [Result, Module] =
                    LogicalDevice.createShaderModule(
                        {.codeSize = ShaderInfo.Spv.size(), .pCode = ShaderInfo.Spv.data()});
            VK_CHECK(Result, vk::Result::eSuccess, "Failed to create shader module");

            ShaderStageInfo.push_back(
                {
                    .stage = static_cast<vk::ShaderStageFlags>(ShaderInfo.Layout.Stage),
                    .module = Module,
                    .pName = "main"
                });
            ...
            if (ShaderInfo.Layout.PushConstantSize != 0)
            {
                ...
                PushConstantInfo.push_back({
                    .stageFlags = static_cast<vk::ShaderStageFlags>(ShaderInfo.Layout.Stage),
                    .offset = PC_Offset,
                    .size = ShaderInfo.Layout.PushConstantSize
                });
                PC_Offset += ShaderInfo.Layout.PushConstantSize;
            }
        }
        vk::PipelineLayoutCreateInfo PipelineLayoutInfo{
            .setLayoutCount = static_cast<uint32_t>(CreateInfo.Shader.size()),
        ...
        std::tie(Result, Pipeline) = LogicalDevice.createGraphicsPipeline({
            ...
            .renderPass = CreateInfo.Renderpass.Get(),
        });
        VK_CHECK(Result, vk::Result::eSuccess, "Failed to create graphics pipeline");
